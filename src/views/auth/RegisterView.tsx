...
export const RegisterView: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  return (
    <div style={{ padding: '24px', backgroundColor: '#0B0F17', minHeight: '100vh', color: '#F8FAFC', display: 'flex', flexDirection: 'column', justifyContent: 'center', boxSizing: 'border-box' }}>
      ...
      <p style={{ color: '#94A3B8', textAlign: 'center', marginTop: '24px' }}>Já possui uma conta?{' '}<span style={{ color: '#06B6D4', cursor: 'pointer', textDecoration: 'underline' }} onClick={() => onNavigate('login')}>Entrar</span></p>
      <p style={{ color: '#64748B', textAlign: 'center', marginTop: '12px', cursor: 'pointer', fontSize: '0.9rem' }} onClick={() => onNavigate('landing')}>Voltar ao início</p>
    </div>
  );
};
