...
export const LoginView: React.FC<{ onNavigate: (page: string) => void }> = ({ onNavigate }) => {
  ...
  return (
    <div style={{ padding: '24px', backgroundColor: 'var(--bg)', minHeight: '100vh', color: 'var(--text)', display: 'flex', flexDirection: 'column', justifyContent: 'center', boxSizing: 'border-box' }}>
      ...
      <p style={{ color: 'var(--text-secondary)', marginBottom: '24px' }}>Digite seus dados para acessar sua conta</p>
      ...
      <input type="email" placeholder="E-mail" value={email} onChange={(e) => setEmail(e.target.value)} style={{ padding: '14px', borderRadius: '10px', backgroundColor: 'var(--surface)', border: '1px solid var(--border)', color: '#FFF', marginBottom: '16px' }} />
      <input type="password" placeholder="Senha" value={password} onChange={(e) => setPassword(e.target.value)} style={{ padding: '14px', borderRadius: '10px', backgroundColor: 'var(--surface)', border: '1px solid var(--border)', color: '#FFF', marginBottom: '16px' }} />
      ...
      <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', cursor: 'pointer', textAlign: 'center', marginTop: '8px' }} onClick={() => onNavigate('landing')}>
