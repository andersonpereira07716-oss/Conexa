...
export const ProfileProvisorioView: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      if (!user) return;
      const { count, error } = await supabase.from('posts').select('*', { count: 'exact', head: true }).eq('user_id', user.id);
  ...
  return (
    <div style={{ padding: '20px', color: '#F8FAFC', paddingBottom: '80px', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-around', backgroundColor: '#161F30', border: '1px solid #1E293B', padding: '16px', borderRadius: '12px', marginBottom: '24px', textAlign: 'center' }}>
        ...
        <div><strong style={{ display: 'block', fontSize: '1.1rem' }}>0</strong><span style={{ fontSize: '0.75rem', color: '#94A3B8' }}>Seguidores</span></div>
        <div><strong style={{ display: 'block', fontSize: '1.1rem' }}>0</strong><span style={{ fontSize: '0.75rem', color: '#94A3B8' }}>Seguindo</span></div>
      </div>
